...
def getPaths(filename):
    ...
    for record in records:
        ...
        if prev_goal != None and prev_goal != f_goal:
            if len(sequences[str(eventCount)]) == 0:
                sequences.pop(str(eventCount))
            eventCount += 1

        if str(eventCount) not in sequences:
            sequences[str(eventCount)] = [[object, duration]]
        else:
            sequences[str(eventCount)].append([object, duration])
